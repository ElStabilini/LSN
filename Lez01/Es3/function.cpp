...
/*========================================================================================
	FUNZIONI CALCOLO PI
=========================================================================================*/
//funzione per intersezione con una linea - assumo le linee con coordinate x intere, separate da una unità


	
//FUNZIONE PER CALCOLO PI GRECO
template<typename T> void BlochStatPi(int Nth, double L, int Lbox, vector<T>& x1, vector<T>& x2, vector<T>& y2, vector<T>& pi_ave, vector<T>& pi_ave2, vector<T>& pi_sum, vector<T>& pi_sum2, vector<T>& err_prog) {
   ...
   for(int j=0; j<Lbox; j++){ //aggiorno il valore della somma e della somma quadrata
   	double k = j+i*Lbox;
		if ( Lancio(x1.at(k), x2.at(k), y2.at(k))==true )
			succ += 1;
    }
    
   pi_ave.push_back( (2*L*Lbox)/succ ); //probabilità per ciascun box
   pi_ave2.push_back( (pow(pi_ave.at(i),2)) );  
  }
    ...
    pi_sum2.push_back(0);
		...
		for(int j=0; j<i+1; j++) {
		  pi_sum.at(i) += pi_ave.at(j);
    	pi_sum2.at(i) += pi_ave2.at(j);
    }

    pi_sum.at(i)/=(i+1); //Cumulative average
    pi_sum2.at(i)/=(i+1); //Cumulative square average
...
/*bool Lancio(double x1, double x2, double y2) {
	//out2 << x1 << ", " << x2 << ", " << y2 <<", "<< endl;
	bool succ = false; //controllare che questa variabile sia settata su falso
	double DXline = ((int)x1 + 1);
	//out2 << DXline << endl;
	double SXline = (int)x1;
	//out2 << SXline << endl;
	if (x2 > DXline or x2 < SXline) 
		succ = true;			
	return succ; //verificare che sia corretto il return della funzione bool	
};		*/
