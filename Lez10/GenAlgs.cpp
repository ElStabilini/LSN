...
Percorso::Percorso(Random* rnd, string filename) {
...
double Percorso::GetLunghezza() const {
	return m_pos.size();
}

void Percorso::SetVec(vector<Posizione> pos){

	if(pos.size() != m_npos)
		cout << "il vettore di posizioni ha dimensione diversa da quella del percorso" << endl;
	
	for(int i=0; i<m_npos; i++) {
		Posizione trash;
		trash.x = pos.at(i).x;
		trash.y = pos.at(i).y;
		m_pos.push_back(trash);
	}
}
...
Percorso* Popolazione::GetMap() const {
	return m_p;
}
...
void Popolazione::Sostituisci(int k, Individuo I) {
	m_pop.at(k) = I;
}
