...
// distance i-j in pbc
  ...
  walker[ip] = 16*p/vol + rho*walker[it]; //Pressure - aggiunto mar 3 mag 2022, 12:19:27 
...
void Averages(int iblk) { //Print results for current block
   ...
   ofstream Epot, Ekin, Etot, Temp, Press; //press aggiunto - mar 3 mag 2022, 12:20:11 
   const int wd=12;
    ...
    Press.open("output_press.dat",ios::app); //press aggiunto - mar 3 mag 2022, 12:24:29
...
void Preprint(int i) {
	...
	if(i%100 == 0) {
	cout << "termalizzazione step: " << i << endl;
	cout << "----------------------------" << endl << endl;
	}
	
	ofstream PreprintTempG;
	const int wd=12;
	
	stima_temp = walker[it];
	
	PreprintTempG.open("PreprintTempG.dat",ios::app);
	PreprintTempG << i << setw(wd) << stima_temp << endl;
